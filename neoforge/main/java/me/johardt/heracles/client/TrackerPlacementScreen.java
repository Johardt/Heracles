...
public final class TrackerPlacementScreen extends Screen {
    ...
    private HeraclesClientOptions.TrackerAnchor selected;
    ...
    public TrackerPlacementScreen(Screen parent) {
        super(Component.translatable("screen.heracles.tracker_placement"));
        this.parent = parent;
        this.selected = HeraclesClientOptions.trackerAnchor();
    }
    ...
    protected void init() {
        ...
        HeraclesClientOptions.TrackerAnchor[] anchors = HeraclesClientOptions.TrackerAnchor.values();
        for (int index = 0; index < anchors.length; index++) {
            HeraclesClientOptions.TrackerAnchor anchor = anchors[index];
            int column = index / 4;
            int row = index % 4;
        ...
        Button done = Widgets.button(widget -> {
            ...
            widget.withRenderer(WidgetRenderers.text(Component.translatable("gui.heracles.done")));
    ...
    public void extractRenderState(
        ...
        graphics.text(
            font,
            Component.translatable("screen.heracles.tracker_placement.instructions"),
            textLeft,
            panelTop + 19,
        ...
        if (preview.width() > 0 && preview.height() > 0) {
            ...
            graphics.text(font, Component.translatable("hud.heracles.pinned_quests.title"), preview.x() + 6, preview.y() + 2, ClientThemeLoader.active().tracker().title(), true);
            int questY = preview.y() + 13;
            graphics.text(
                font,
                Component.translatable("screen.heracles.tracker.preview_quest_title"),
                preview.x() + 6,
                questY,
            ...
            graphics.text(
                font,
                Component.translatable("screen.heracles.tracker.preview_task_one"),
                preview.x() + 10,
                questY + 12,
            ...
            graphics.text(
                font,
                Component.translatable("screen.heracles.tracker.preview_task_one_progress"),
            ...
            graphics.text(
                font,
                Component.translatable("screen.heracles.tracker.preview_task_two"),
                preview.x() + 10,
                questY + 23,
            ...
            graphics.text(
                font,
                Component.translatable("screen.heracles.tracker.preview_task_two_progress"),
                preview.x() + preview.width() - 6 - font.width(
                    Component.translatable("screen.heracles.tracker.preview_task_two_progress")
                ),
                questY + 23,
    ...
    private void select(HeraclesClientOptions.TrackerAnchor anchor) {
        selected = anchor;
        HeraclesClientOptions.setTrackerAnchor(anchor);
        rebuildWidgets();
    }
    ...
    private static String anchorKey(HeraclesClientOptions.TrackerAnchor anchor) {
        return "screen.heracles.tracker.anchor." + anchor.name().toLowerCase(java.util.Locale.ROOT);
    }
}
