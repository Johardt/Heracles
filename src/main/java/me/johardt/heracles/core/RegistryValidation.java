...
public final class RegistryValidation {
    ...
    private static void validateTasks(String questId, JsonObject tasks, String path, Resolver resolver, List<QuestDiagnostics.Diagnostic> diagnostics) {
        tasks.entrySet().forEach(entry -> {
            ...
            switch (type) {
                case "heracles:item", "heracles:item_interaction", "heracles:item_use" -> check(questId, task, taskPath, "item", Target.ITEM, resolver, diagnostics);
                case "heracles:kill_entity", "heracles:entity_interaction" -> check(questId, task, taskPath, "entity", Target.ENTITY, resolver, diagnostics);
                case "heracles:block_interaction" -> check(questId, task, taskPath, "block", Target.BLOCK, resolver, diagnostics);
                case "heracles:biome" -> check(questId, task, taskPath, "biomes", Target.BIOME, resolver, diagnostics);
                ...
                case "heracles:structure" -> check(questId, task, taskPath, "structures", Target.STRUCTURE, resolver, diagnostics);
                case "heracles:stat" -> check(questId, task, taskPath, "stat", Target.STAT, resolver, diagnostics);
                case "heracles:advancement" -> check(questId, task, taskPath, "advancements", Target.ADVANCEMENT, resolver, diagnostics);
                case "heracles:recipe" -> check(questId, task, taskPath, "recipes", Target.RECIPE, resolver, diagnostics);
                default -> { }
            }
            if ("heracles:composite".equals(type)) validateTasks(questId, object(task, "tasks"), taskPath + ".tasks", resolver, diagnostics);
        });
    }
    ...
    private static void validateRewards(String questId, JsonObject rewards, String path, Resolver resolver, List<QuestDiagnostics.Diagnostic> diagnostics) {
        rewards.entrySet().forEach(entry -> {
            ...
            } else if ("heracles:loottable".equals(type)) {
                checkValue(questId, string(reward, "loot_table", ""), rewardPath + ".loot_table", Target.LOOT_TABLE, resolver, diagnostics);
            }
            if ("heracles:selectable".equals(type)) validateRewards(questId, object(reward, "rewards"), rewardPath + ".rewards", resolver, diagnostics);
        });
    }
